...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const tmpUsers = await mongoose.models.user.find({ id_auteur: { $in: (channel.users || []) } }).sort({ score: -1, nom: 1 }).limit(3)

    const [pp1, pp2, pp3] = await Promise.all(tmpUsers.map(user => getProfilePicture(user.id_auteur)))

    const { data } = await axios.post('http://localhost:11871/api/render', { html, content: { name1: tmpUsers[0].nom, pp1, name2: tmpUsers[1].nom, pp2, name3: tmpUsers[2].nom, pp3 }, width: 626, height: 352 }, { responseType: 'arraybuffer' })
