	...
	fetchChallenges: async function(client, channelIds) {
		...
		while (fetchContinue) {
			...
			for (const page of pages) {
				...
				if (f) ret = await mongoose.models.challenge.updateOne({ id_challenge: page.id_challenge }, reqPage.data)
				else {
	...
	updateUsers: async function(client, channelIds) {
		...
		for await (const user of mongoose.models.user.find()) {
			try {
				...
				const toCheck = [
					{ key: 'validations', id: 'id_challenge', value: 'date', title: 'Nouveaux challenges validés par ' },
					{ key: 'solutions', id: 'id_solution', value: 'url_solution', title: 'Nouvelles solutions ajoutées par ' },
					{ key: 'challenges', id: 'id_challenge', value: 'url_challenge', title: 'Nouveaux challenges créés par ' }
				]
